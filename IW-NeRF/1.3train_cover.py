...
my_model.load_state_dict(torch.load('./models/big.pth'))

small_model = SmallModel(hidden_dim=128).to('cuda')

# 冻结参数
# 冻结需要保持不变的参数
with torch.no_grad():
    for new_name, new_param in my_model.named_parameters():
...
n_steps = 50000 # Due to limited computation power
